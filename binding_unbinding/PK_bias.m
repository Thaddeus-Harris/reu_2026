...
for i = 2:(T+1)
  ...
  if (r_sum * W_2(i) < r_bind)
	...
	elseif (r_bind < r_sum * W_2(i) && r_sum * W_2(i) <= r_bind + r_unbind)
		...
		theta = 2 * pi * rand(1);
		X(2,unbound_index,i) = X(2,bound_index,i) + 0.005;
	...
	flip = W_5(:,i) < (lambda * tau);
	%random angle for 2d random walk
	theta_RW = rand(1,N_x) * 2 * pi;
	theta_PK = rand(1,length(flip)) * 2 * pi;
	random_steps_x = cos(theta_RW) *  D * tau;
	random_steps_y = sin(theta_RW) *  D * tau;
	random_velocity_angle_x = cos(theta_PK) *  v_r;
	random_velocity_angle_y = sin(theta_PK) *  v_r;

	X(4,flip,i) = random_velocity_angle_x(flip);
	X(5,flip,i) = random_velocity_angle_y(flip);

	%Adjustments from velocity
	X(2,:,i) = mod(X(2,:,i) + X(4,:,i) * tau,L);
	X(3,:,i) = mod(X(3,:,i) + X(5,:,i) * tau,L);

	%Adjustments from random walk
	X(2,:,i) = mod(X(2,:,i) + random_steps_x, L);
	X(3,:,i) = mod(X(3,:,i) + random_steps_y, L);
...
for i = 2:(T+1)
  ...
  if (r_sum * W_4(i) < r_bind)
	...
	elseif (r_bind < r_sum * W_4(i) && r_sum * W_4(i) <= r_bind + r_unbind)
	...
	%random angle for 2d random walk
	theta = rand(1) * 2 * pi;
	random_steps_x = cos(theta) * ((2 * randi(2, 1, length(Y(2,bound,i)))) - 3) * D * tau;
	random_steps_y = sin(theta) * ((2 * randi(2, 1, length(Y(2,bound,i)))) - 3) * D * tau;
