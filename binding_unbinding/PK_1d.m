...
k_bind = 0.0001; % Binding rate constant (Transitions from uniform to clusters at k_bind/k_fee
k_unbind = 0.7; % Unbinding rate constant
k_feedback = 2.0; % Feedback rate constant
lambda = 0.5;
L = 1; % Length of cell membrane
v_x = 0.01; % Velocity for random walk
...
for i = 2:(T+1)
  ...
  flip = W_4(:,i) < (lambda * tau); % Logical mask that picks out which particles flip direction
	X(3,flip,i) = -X(3,flip,i);
	X(2,:,i) = mod(X(2,:,i) + (v_x * tau), L);
