...
N = 100; T = 1000; k_bind = 0.5; k_unbind = 0.4; L = 1; v_x = 0.005; num_iids = 20;
...
for i = 1:num_iids
	...
	for t = 2:(T+1)
		...
		r_bind = k_bind*(1-A(1,t-1,i));
		r_unbind = k_unbind*A(1,t-1,i);
		...
		for n = 1:N
			switch X(1,n,t,i)
				case 0
					X(2,n,t,i) = X(2,n,t-1,i);
				case 1
					X(2,n,t,i) = mod( (X(2,n,t-1,i) + (2 * randi(2) - 3) * v_x * tau), L);
			end
		end
		% Updating proportion and time for A
		A(:,t,i) = [(sum(X(1, :,t,i)) / N), (A(2, t-1,i) + tau)];
	end
end
...
X_all = reshape(X(2,:,T,:), 1, num_iids*N);
subplot(1,2,2);
hist(X_all,20);
